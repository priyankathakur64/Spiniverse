...
import { TwitterShareButton, FacebookShareButton, WhatsappShareButton, LinkedinShareButton, TwitterIcon, FacebookIcon, WhatsappIcon, LinkedinIcon } from 'react-share';
import {
Grid,
Box,
Button,
...
import { login, register, getUserDetails, updateSpins, listUsers } from "../actions/userActions";
...
{user?.spins > 0 ? `Free Spin Left: ${user.spins} ${user.spins > 1 ? "s" : ""} ` : "No Free Spins Left"}
</Text>
<Tooltip
...
<AiOutlineEye />
    ...
    {bonusSpins > 0 ? `Bonus Spin Left: ${bonusSpins} ${bonusSpins > 1 ? "s" : ""} ` : "No Bonus Spins"}
    </Text>
    <Tooltip
    ...
    <Button
      ...
      _active={{ bg: "transparent" }}
      _focus={{ boxShadow: "none" }}
      onClick={() => {/* handle the buy action here */}}
    >
